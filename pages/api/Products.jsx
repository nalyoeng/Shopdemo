  ...
  return (
    <div className='w-full min-h-auto p-3 mt-3'>
      ...
      <div className='flex items-center gap-3 hover:text-blue-700 ps-3'>
        <a href='/devices' className='lg:text-4xl md:text-3xl text-2xl font-medium'>
          New Collection
        </a>
        <svg
          className='mt-3'
      ...
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4'>
        {products.map((item) => (
          <div
            key={item.id}
            className='w-full h-auto border-2 border-gray-400 rounded-2xl  shadow-md hover:shadow-xl hover:scale-95 transition-transform duration-200'
          >
            {/* Product Image */}
            <img
              className='w-90 h-80 rounded-t-2xl'
              src={item.img}
              alt={item.name}
            />
